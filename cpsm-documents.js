document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateRoleMode = () => {
    ...
    clientSelect.value = "Momentum Data Client";
    clientSelect.disabled = isClient;

    if (isClient) {
      visibilitySelect.value = "Uploaded by Client";
      statusSelect.value = "Pending Review";
      roleHelp.textContent = "Client users can upload only to their own client account. Client assignment is locked.";
    } else {
      roleHelp.textContent = "Internal users can select a client and control document status, visibility, category, type, and expiration.";
    }
  };
  ...
  form.addEventListener("submit", (event) => {
    ...
    row.innerHTML = `
      ...
      <td><span class="documents-pill ${pillClass(status)}">${escapeHtml(status)}</span></td>
      <td>${escapeHtml(expiration || "No expiration")}</td>
      <td>${escapeHtml(visibility)}</td>
      <td><button class="documents-action" type="button">View</button></td>
    `;
