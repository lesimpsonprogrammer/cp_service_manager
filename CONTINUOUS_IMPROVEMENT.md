...
|------------|--------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|--------|
| 2026-09-08 | No clean way to revoke access or purge stale data — the only "removal" option was hard-deleting a member's entire account, and there was no data retention/archival process at all | No status concept on org_members, no archive tier for removed data | Added active/suspended/removed status with a recycle bin (restorable), plus org-scoped, categorized archive storage with a configurable retention window (7yr default, grounded in FLSA/IRS/state research) — archive-first, no hard deletion | Done |
| 2026-09-08 | Settings page had accumulated Workspace, Members, Appearance, Doc categories, Invites, Pending signups, and Developers all on one long scrolling page | No dedicated pages per settings area — everything grew on a single index route | Split into an index panel linking to `/settings/team` (Access Permissions), `/settings/appearance`, `/settings/docs`, and `/settings/developers` | Done |
